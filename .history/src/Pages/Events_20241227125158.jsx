...
const Event = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate or process the data here if needed

    try {
      const response = await fetch("https://api.web3forms.com/submit", {
        method: "POST",
        body: new FormData(e.target), // Sending form data as FormData
      });

      const data = await response.json();

      if (data.success) {
        // If successful, reset the registration state and handle the success
