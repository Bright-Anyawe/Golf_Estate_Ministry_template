...
<section className="upcomingEvent container flex flex-col justify-center items-center gap-10 bg-gradient-to-r from-teal-500 via-indigo-600 to-purple-700 p-8 py-16 text-center md:text-left">
  ...
  {upcomingEvent ? (
    <div className="eventCard bg-gradient-to-r from-indigo-500 via-purple-600 to-blue-500 text-white shadow-2xl rounded-lg p-8 flex flex-col items-center text-center md:flex-row gap-6">
      ...
      <button className="bg-indigo-700 text-white p-4 rounded-lg mt-6 md:mt-0 hover:bg-indigo-800 transition-all duration-300">
        Learn more
      </button>
    </div>
  ) : (
    <p className="text-white text-xl">No upcoming events at the moment.</p>
  )}
</section>
...
<section className="latestSermon container flex flex-col justify-center items-center gap-10 bg-gradient-to-r from-gray-300 via-yellow-300 to-blue-600 p-8 py-16 text-center md:text-left">
  <h2 className="text-4xl font-bold mb-6 text-gray-900 shadow-lg">Current Teachings of the Word</h2>
  ...
  <div className="teachingContainer flex flex-col md:flex-row justify-center items-center gap-10 w-full">
    ...
    <div className="flex flex-col justify-center items-center gap-6 bg-white bg-opacity-90 p-8 rounded-xl shadow-2xl border border-gray-300 w-full md:w-1/2">
      <h2 className="text-3xl font-bold mb-4 text-indigo-700">Latest Sermon</h2>
      <div>
        <h3 className="text-2xl font-semibold text-gray-800">{latestSermon.title}</h3>
        <p className="text-lg text-gray-700 mt-2">{latestSermon.description}</p>
        <p className="font-bold text-lg text-indigo-600 mt-4">Speaker: {latestSermon.speaker}</p>
        <p className="text-lg text-gray-600 mt-2">Date: {new Date(latestSermon.date).toLocaleDateString()}</p>
      </div>
    </div>

    {/* Upcoming Bible Study Section */}
    <div className="flex flex-col justify-center items-center gap-6 bg-white bg-opacity-90 p-8 rounded-xl shadow-2xl border border-gray-300 w-full md:w-1/2">
      <h2 className="text-3xl font-bold mb-4 text-indigo-700">Upcoming Bible Study</h2>
      <div>
        <h3 className="text-2xl font-semibold text-gray-800">{latestBibleStudy.topic}</h3>
        <p className="text-lg text-gray-700 mt-2">{latestBibleStudy.description}</p>
        <p className="font-bold text-lg text-indigo-600 mt-4">Memory Verse: {latestBibleStudy.memoryVerse}</p>
        <p className="text-lg text-gray-600 mt-2">Main Text: {latestBibleStudy.mainText}</p>
      </div>
    </div>
  </div>
