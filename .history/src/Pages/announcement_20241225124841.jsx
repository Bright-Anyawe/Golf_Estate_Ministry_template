  ...
  <div className="flex flex-wrap gap-5 items-center justify-center">
    {weeklyAnnouncements.map((announcement) => (
      <div
        ...
        className="bg-white shadow-lg rounded-xl p-6 flex flex-col items-center transform hover:scale-105 transition-transform duration-300 ease-in-out hover:shadow-2xl"
      >
        <h3 className="text-xl font-bold mb-2 text-blue-600">{announcement.title}</h3>
        <p className="text-sm text-gray-500 mb-2 italic">{announcement.date}</p>
        <p className="text-gray-700 text-center">{announcement.description}</p>
      </div>
    ))}
  </div>
</div>
